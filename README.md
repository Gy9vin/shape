...
Раз в 10 секунд он замеряет скорость каждого IP. Если она держится выше порога
дольше заданного времени — адрес получает персональный низкий лимит на срок.
Всё настраивается в меню: Автоограничение.

```
  Порог               : 8 Mbit/s (80% от лимита) Держится 5 мин
  Скорость нарушителя : 1 Mbit/s на 60 мин
```

Счётчик с допуском: замер выше порога прибавляет очко, ниже — отнимает. Короткие
провалы на буферизацию штраф не отменяют, но и сёрфинг с паузами очков не
накопит.

### Почему YouTube не попадёт под штраф

Битрейт 1080p — 3–4 Мбит/с. Плеер набивает буфер рывками: секунд двадцать на
полной скорости, потом короткие всплески. Средняя за пять минут выходит 30–40%
от канала в 10 Мбит/с. Пятиминутное видео 1080p весит 100–150 МБ, а чтобы
удержать 8 Мбит/с пять минут, нужно скачать 300 МБ — неоткуда.

Проверено симуляцией на восьми сценариях:

| Сценарий | Пик счётчика | Результат |
|---|---|---|
| YouTube 1080p, телефон | 4 из 30 | чисто |
| Соцсети, Telegram, почта | 0 из 30 | чисто |
| Видеозвонок | 0 из 30 | чисто |
| YouTube 4K на большом экране | 30 из 30 | штраф через 10 мин |
| Торрент | 30 из 30 | штраф через 5 мин |
| Скачивание ISO 5 ГБ | 30 из 30 | штраф через 5 мин |

**Обратная сторона.** L3/L4 не видит содержимого, поэтому разовая большая
закачка выглядит как торрент. Если хочешь разрешить, скажем, файл на 700 МБ —
подними «сколько минут нагрузки до штрафа» до 10–15: торрент качает часами и
всё равно попадётся, а одиночная закачка успеет закончиться.

**1 Мбит/с нарушителю хватает,** чтобы написать в поддержку: переписка в
мессенджере ест 10–50 кбит/с, голосовой звонок в Telegram — до 60 кбит/с.
Не работает только видео и загрузка файлов.

Ограниченные видны в меню отдельным пунктом со счётчиком и остатком времени,
снять можно поимённо или всех сразу. Штрафы переживают перезапуск сервиса и
...
shaperctl.py guard --enable --sustain 5 --penalty-mbps 1 --penalty-min 60
...
/etc/shaper/penalties.json кто ограничен и до какого времени
```
