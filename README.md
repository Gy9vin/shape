...
### Второй путь: торрент без раздачи

Клиент с выключенной раздачей отдаёт лишь запросы блоков — десятые доли мегабита
мелкими пакетами. Обязательное условие он не проходит, признаки не срабатывают.

И это честно: **торрент без раздачи с точки зрения сети неотличим от обычной
тяжёлой закачки.** Никакой признак его не выдаст, потому что выдавать нечего.

Выдаёт объём. Отдельный, независимый путь к штрафу: **50 ГБ скачивания за
сутки**, отдача не учитывается вовсе. Настраивается в меню, `0` выключает.

| Сценарий за 13 часов | Выкачано | Результат |
|---|---|---|
| Качает на потолке без раздачи | 50 ГБ | штраф через 11 ч |
| Покачал 3 часа и остановился | 14 ГБ | чисто |
| Скачал два ISO по 5 ГБ | 10 ГБ | чисто |
| Стриминг 4K шесть часов | 24 ГБ | чисто |
| YouTube 1080p весь вечер | 8 ГБ | чисто |

Пятьдесят гигабайт при лимите 10 Мбит/с — это одиннадцать часов непрерывной
закачки на полной скорости. Ни один сценарий просмотра столько не даёт.
